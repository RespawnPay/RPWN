...
const char * const BITCOIN_CONF_FILENAME = "RPWN.conf";
const char * const BITCOIN_PID_FILENAME = "RPWNd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "RPWN" is a composite category enabling all RPWN-related debug output
            if(ptrCategory->count(string("RPWN"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RPWNCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RPWNCore
    // Mac: ~/Library/Application Support/RPWNCore
    // Unix: ~/.RPWNcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RPWNCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RPWNCore";
#else
    // Unix
    return pathRet / ".RPWNcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty RPWN.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override RPWN.conf
